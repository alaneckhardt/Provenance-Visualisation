	...
	force = function() {
		...
		function tick() {
			...
			for (i = 0; i < m; ++i) {
				...
				if (l = (x * x + y * y)) {
					l = this.alpha * strengths[i] * ((l = Math.sqrt(l)) - distances[i])
							/ l;
					x *= l;
					y *= l;
			...
			if (charge) {
				d3_layout_forceAccumulate(q = quadtree(nodes), this.alpha, charges);
				i = -1;
				while (++i < n) {
					if (!(o = nodes[i]).fixed) {
						q.visit(repulse(o));
